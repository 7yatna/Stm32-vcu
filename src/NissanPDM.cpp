...
void NissanPDM::DecodeCAN(int id, uint32_t data[2])
{
    ...
    {
        Param::SetInt(Param::CanAct, 1);//PDM wants to talk so we need to talk back to it.
    }
...
void NissanPDM::Task10Ms()
{
    ...
    if (opmode != MOD_RUN) //Outside of Run PDM sends VCM 10ms task else sent by leafinv.cpp
    {
